...
def solve_timetable():
    ...
    can_solve_slot(time_table, module_tutor_pairs, 1, 18750)
...
def can_solve_slot(time_table, pairs, slot, bound, cost=0):
    """
    ...
    for pair in pairs:
        if can_assign_pair(time_table, day, pair):
            ...
            new_cost = cost + calc_node_cost(time_table, day, pair)
            if new_cost > bound:
                continue
            
            pruned_pairs = forward_checking(pair, pairs)

            if can_solve_slot(time_table, pruned_pairs, slot + 1, bound, new_cost):
                return True
